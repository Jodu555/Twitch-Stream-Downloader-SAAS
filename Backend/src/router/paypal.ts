...
import { z } from 'zod';
import { resetAccountToTier } from './auth';
...
router.post('/api/v1/paypal/captureOrder', async (req, res, next) => {
    try {
        ...
        const { invoiceID, orderID } = bodyData.parse(req.body);
        ...
        if (order.status == OrderStatus.Approved || order.status == OrderStatus.Completed) {
            ...
            if (invoiceActionIntent == 'setRank') {
                ...
                const account = await database.get<Account>('accounts').getOne({ UUID: invoice.userUUID });

                await database.get<Account>('accounts').update({ UUID: invoice.userUUID }, {
                    subscription_type: invoiceActionData as SubscriptionTypes,
                    last_renewed: Date.now(),
                    first_subscribed: account.first_subscribed || Date.now(),
                });
            }
